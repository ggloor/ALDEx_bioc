...
##Pass to ALDEx2
aldexScaleSim.fit <- aldex(countdata, as.character(rdat$Condition), gamma = scale.samps, mc.samples = 2000)
...
aldexScaleSim.fitCLR <- aldex(countdata, as.character(rdat$Condition), gamma = 1e-3, mc.samples = 2000)
 
sig.aldexScaleSimCLR = aldexScaleSim.fitCLR %>% filter(wi.eBH <= 0.05)
head(sig.aldexScaleSimCLR)
dim(sig.aldexScaleSimCLR)

aldexScaleSim.fitCoDA <- aldex(countdata, as.character(rdat$Condition), gamma = 10, mc.samples = 2000)
...
senAnalysis = aldex.senAnalysis(x, gamma = c(1e-3, .1, .25, .5, 1, 2, 3))
