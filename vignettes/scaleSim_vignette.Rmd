...
##Pass to ALDEx2
aldexScaleSim.fit <- aldex(countdata, as.character(rdat$Condition), scale.lambda=0.5, scale.mu =1, mc.samples = 2000)
...
aldexScaleSim.fitCLR <- aldex(countdata, as.character(rdat$Condition), scale.lambda = 1e-3, mc.samples = 2000)
 
sig.aldexScaleSimCLR = aldexScaleSim.fitCLR %>% filter(wi.eBH <= 0.05)
head(sig.aldexScaleSimCLR)
dim(sig.aldexScaleSimCLR)

aldexScaleSim.fitCoDA <- aldex(countdata, as.character(rdat$Condition), scale.lambda = 10, mc.samples = 2000)
...
```{r getScaleSamps}
x <- aldex.clr(countdata, as.character(rdat$Condition), scale.lambda = 10, mc.samples = 2000)
...
```{r senAnalysis}
x <- aldex.clr(countdata, as.character(rdat$Condition), scale.lambda = 1e-3, mc.samples = 2000)
