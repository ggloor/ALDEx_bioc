...
sig.aldex = aldex.fit %>% filter(we.eBH <= 0.05)
head(sig.aldex)
dim(sig.aldex)
```
...
```{r ALDEX2ScaleSimOtherModsSamples}
mu.vec = 1/apply(rdat[,-1],1,gm)
##Samples from the CLR model
scale.sampsCLR = t(sapply(mu.vec, FUN = function(mu){rlnorm(2000, mu, sdlog = 1e-3)}))
##Samples from the CoDA model
scale.sampsCoDA = t(sapply(mu.vec, FUN = function(mu){rlnorm(2000, mu, sdlog = 10)}))
```
...
aldexScaleSim.fitCLR <- aldex(countdata, as.character(rdat$Condition), scale.samples = scale.sampsCLR, mc.samples = 2000)
 
sig.aldexScaleSimCLR = aldexScaleSim.fitCLR %>% filter(we.eBH <= 0.05)
head(sig.aldexScaleSimCLR)
dim(sig.aldexScaleSimCLR)

aldexScaleSim.fitCoDA <- aldex(countdata, as.character(rdat$Condition), scale.samples = scale.sampsCoDA, mc.samples = 2000)
 
sig.aldexScaleSimCoDA = aldexScaleSim.fitCoDA %>% filter(we.eBH <= 0.05)
...
```{r getScaleSamps}
x <- aldex.clr(countdata, as.character(rdat$Condition), scale.samples = scale.sampsCoDA, mc.samples = 2000)
