...
aldexScaleSim.fitCLR <- aldex(countdata, as.character(rdat$Condition), scale.samples = 1e-3, mc.samples = 2000)
 
sig.aldexScaleSimCLR = aldexScaleSim.fitCLR %>% filter(wi.eBH <= 0.05)
head(sig.aldexScaleSimCLR)
dim(sig.aldexScaleSimCLR)

aldexScaleSim.fitCoDA <- aldex(countdata, as.character(rdat$Condition), scale.samples = 10, mc.samples = 2000)
...
```{r getScaleSamps}
x <- aldex.clr(countdata, as.character(rdat$Condition), scale.samples = 10, mc.samples = 2000)
...
Now, we are going to do a small experiment to make sure that the CLR model is return similar effect sizes as the `selex` data.
...
selex.scale <- matrix(rlnorm(128*14, 1, sdlog=0.5), nrow=14)
x.s <- aldex.clr(selex, conds, scale.samples=selex.scale)
...
x.s <- aldex.clr(selex, conds, scale.samples=1e-3)
x.s.e <- aldex.effect(x.s)
sig <- which(abs(x.e$effect) > 2)
sig.s <- which(abs(x.s.e$effect) > 2)
