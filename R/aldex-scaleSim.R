...
interpretGamma <- function(clr){
  ...
  if(is.vector(clr@conds)){
    scale.comb <- clr@scaleSamps
    log.scale <- log2(scale.comb)
    ...
    return(data.frame("mean" = mean(theta.perp), "p2.5" = quantile(theta.perp, c(0.025)), "p97.5" = quantile(theta.perp, c(0.975))))
    ...
    return(data.frame("Variable" = colnames(X), "mean" = avg.est, "p2.5" = lower.est, "p97.5" = upper.est))
  } else{
